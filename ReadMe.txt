...
Asignment question attempted: 1 ( KBC Game )
...
2. We have used 10+ functions in our code, to make it as modular as possible as mentioned in the problem statement itself. This also makes the code eaiser to understand and debug for
   the user. Furthermore, we have declared fuction prototypes as well when we could've done without them easily because we were taught that's it's always good practice as it increases
...
equally in the project from day-1 . We had decided the task of each team member beforehand and checked the progress regularly through google meets. Not unlike the fuctions in our code,
...
->Ishan( 2019B3PS0504P ) envisioned the structure of the code and identified the fuctions to be used as well as the procedural prompts guiding the contestant throughout the run of the game.
  ...
  He decided to use file for these purposes and encoded all the above fuctionalities in the code.

->Aryan( 2019B3PS0406P ) thought of adding the option to quit after the first definite level and handeled the entire fuctionality to do so i.e encoded it in the code. He also provided insights in the code
  at various intervals starting from the first draft itself. He handeled the comments throughout the code to explain each and every line clearly to the user.
